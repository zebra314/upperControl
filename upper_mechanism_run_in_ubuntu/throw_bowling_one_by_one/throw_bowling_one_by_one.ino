...
// 3 : takeBowling() , will take one bowling 
// 5 : pre_taking_bowling() , send before taking bowling , upper mechanism will lift up the pusher and stretch downDC 
// 6 : horizon_pusher() , send after taking bowling , upper mechanism will make the pusher change to horziontal position
...
void throwing_basketball_three(int& time , int& status)
{
    ...
    if(time == 1)
    {
        ...
        else if(status == 3 and duration < 8500 ) //秒數待訂
        {
        ...
        else if(status == 3 and duration > 8500)
        {   
            MOTstatus = 1;
            downDC_task(MOTstatus); 
            topStepper_task(MOTstatus);
